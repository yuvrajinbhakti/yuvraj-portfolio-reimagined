...
export const caseStudies = [
  ...
  },
  {
    ...
    metrics: [
      ...
      { value: '66', label: 'tests behind it now, none of which existed' },
    ],
    sections: [
      {
        heading: 'The problem',
      ...
      },
      {
        ...
        body:
          'Encrypt every file at rest with AES-256-GCM, which authenticates the ciphertext as well as hiding it, so tampering is detected rather than silently decrypted. Put access behind JWT authentication with per-user roles. Write every access to an audit log in MongoDB, so the system can say exactly who touched a document and when. Expire files on a schedule rather than leaving them forever. Hand somebody a link to one file — bounded by an expiry, a download count, optionally a password or a named recipient — without making them sign up for an account. That is the product, and every part of it now runs, which is a sentence I could not have written when I started reading this code.',
      },
      {
