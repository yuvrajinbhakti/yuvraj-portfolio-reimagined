...
export const caseStudies = [
  ...
  },
  {
    ...
    metrics: [
      ...
      { value: '14', label: 'months the deploy failed with no failure shown' },
      { value: '79', label: 'tests behind it now, none of which existed' },
    ],
    sections: [
      {
        heading: 'The problem',
      ...
      },
      {
        ...
        body:
          'Encrypt every file at rest with AES-256-GCM, which authenticates the ciphertext as well as hiding it, so tampering is detected rather than silently decrypted. Put access behind JWT authentication with per-user roles. Write every access to an audit log in MongoDB, so the system can say exactly who touched a document and when. Expire files on a schedule rather than leaving them forever. Hand somebody a link to one file — bounded by an expiry, a download count, optionally a password or a named recipient — without making them sign up for an account. Select several files and take them as one archive, or delete them together. That is the product, and every part of it now runs, which is a sentence I could not have written when I started reading this code.',
      },
      {
      ...
      },
      {
        ...
        body:
          'Having found the encryption never ran, I audited the rest of the server the same way rather than assuming the first stone was the only loose one. Two features this page advertised were not wired to anything. Share links with an expiry: the logic existed in utils/fileSharing.js and looked complete, but its controller file was empty, no route registered it, and the function that enforces expiry, passwords, download limits and email allowlists was called by nothing. TOTP two-factor: the user model has the fields and the methods, utils/twoFactor.js has the implementation, and no route ever reaches it. Two other things were worth fixing rather than describing. The JWT secret had a fallback — process.env.JWT_SECRET or a hard-coded string, written three times — which means any deployment missing that variable signs its tokens with a key published in a public repository, and forging an administrator token becomes copy and paste. There is no default now; a missing, short or placeholder secret stops the process at startup. And the access check called userId.toString() before testing whether a file was public, while the download route allows unauthenticated requests, so public files threw instead of downloading. That one failed closed, so nothing was exposed. It simply did not work, and nothing said so. Share links have since been built, and so have bulk operations, which were a third thing written and unreachable — both are described below. Two-factor and password reset have not been, and stay off this page until they are.',
      },
      {
        heading: 'Building the half that was missing',
        body:
          'Writing the controller was the easy part. Reading the rules it would call was not, because none of them had ever run — and four of them did not hold. generateShareLink never checked ownership, and File.findById returns any document to anyone who knows its id, so any signed-in account could mint a public, unauthenticated download URL for any file in the system. The email restriction read `if (allowedEmails.length > 0 && userEmail)`, so omitting the email skipped the check entirely: a link restricted to one named recipient opened for anybody who simply did not supply an address. The check existed, ran, and permitted exactly what it forbade. The download cap read the count, added one and wrote it back, which loses every race it enters — and a share link is a URL, so two people opening it at once is the normal case rather than the exotic one. And getUserShareLinks returned an empty array unconditionally behind a comment calling it a placeholder, so a page listing your links would have looked identical to having never shared anything.',
      },
      {
        heading: 'A zero is not evidence, and neither is a passing suite',
        body:
          'Sixty-six tests now cover this, against a real Redis rather than a stub — a stub would agree with whatever I believed about INCR and expiry, which is the assumption under test. I checked that they were worth having by reverting each fix and confirming the corresponding test failed, and the HTTP suite drops from twenty-two passing to three if the routes are removed. Then the tests missed something anyway, and the way they missed it is the point. A mistyped share password produced a dead end on the live site instead of a second attempt. The cause was in the logger: logError(message, error) read error.message unconditionally, and eleven call sites pass only a message. Seven of those are the refusal paths in validateShareLink, each inside a try, so the throw was caught by the function\'s own handler and flattened into a generic failure. The password check worked correctly and said "invalid password, ask again"; the logger destroyed that answer on its way out and logged nothing about having done so. My suite could not see it because I had mocked the logger as an empty function, which accepts any arguments at all. A test double more forgiving than the thing it replaces does not test that thing. Both mocks now assert the real signature and the logger is exercised unmocked.',
      },
      {
        heading: 'The same shape, a third time',
        body:
          'Bulk download and bulk delete were the third feature here written and never connected: 248 lines of controller that the router did not import, and a working button in the client posting to a path that returned 404 every time it was pressed. Connecting it found three more bugs that nothing could have caught while it was unreachable, and the first is the one I would want a reviewer to notice. The controller called createBulkDownload(userId, fileIds) against a function declared as (fileIds, userId) — so the database query deciding which files you are allowed to put in the archive had both arguments the wrong way round. It would still have answered 200, with an empty zip, which is exactly the kind of failure that looks like a working feature. The test for it therefore asserts on what the inner function was called with rather than on the response, because the response cannot tell you. The second: utils/bulkOperations.js called require(\'crypto\') twice, inside a file the package declares as an ES module, where require does not exist — a ReferenceError thrown after the archive had already been built and written to disk. The third: the reply handed the browser zipPath, an absolute path on the server, and no URL it could actually fetch, so the component pointed a download link at a field that was never sent. Seventy-nine tests now, up from sixty-six, and each of these was confirmed to fail its test when put back.',
      },
      {
        heading: 'The deploy had been failing silently for fourteen months',
        body:
          'With the API working I went to look at the front end and found there was no page for a share link to land on — the router\'s catch-all sent every one to the dashboard, which bounced to the login screen. The link worked; the destination did not exist. Building it turned up three more things, none of which were mine: the client called /sharing/create, a path the server has never had, so the share button had failed every time it was pressed since it was written; a stylesheet had an unclosed block inside a @keyframes rule, which fails the CSS parse and takes the whole bundle with it, so the client did not compile at all; and three unused variables broke the production build, because CI treats warnings as errors. That last one explained the shape of everything. The deployed site was a build from fourteen months and twenty-one commits earlier. The cause was one line in vercel.json referencing a Vercel Secret that Vercel had removed, so every deployment failed at config validation — before a build container starts. A deployment rejected at validation never becomes a deployment, so the hosting dashboard showed no failures at all, just one very old success. The homepage returned 200 the entire time. It is the same lesson as the encryption, one layer out: the check I would naturally run answered a question I was not asking.',
      },
      {
